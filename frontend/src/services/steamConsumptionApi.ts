...
interface SteamConsumptionCustomResponse {
  success: boolean;
  // `steamConsumptionTotal` is the aggregate (KG). `steamConsumption` used to be that number but is
  // now an object keyed by devID; read the Total, falling back to summing the map / legacy number.
  data?: {
